...
const FINANCE_SOURCES = NEWS_SOURCES.filter((s) => s.category === "finance");

export function FinanceFeed() {
  ...
  const [viewMode, setViewMode] = useLocalStorage<ViewMode>("finViewMode", "grid");
  const [sortBy, setSortBy] = useLocalStorage<SortOption>("finSortBy", "newest");
  ...
  const load = useCallback(async (isManual = false) => {
    try {
      ...
      const res = await fetch("/api/news?category=finance&limit=80");
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-nowrap gap-1.5 overflow-x-auto pb-1 scrollbar-none">
        <button
          ...
          style={!activeSource && !showBookmarks ? { background: "#10b981" } : { background: "var(--surface-muted)", color: "var(--text-secondary)" }}
        >All</button>
        {FINANCE_SOURCES.map((source) => (
          <button
            key={source.slug}
            onClick={() => { setActiveSource(activeSource === source.slug ? null : source.slug); setShowBookmarks(false); }}
            className={`flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-medium border transition-all ${activeSource === source.slug ? "text-white border-[#10b981]" : "border-[var(--border)] hover:border-[#10b981]/40"}`}
            style={activeSource === source.slug ? { background: "#10b981" } : { background: "var(--surface-card)", color: "var(--text-secondary)" }}
          >
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="py-20 text-center text-sm" style={{ color: "var(--text-tertiary)" }}>
          {showBookmarks ? "No bookmarked Finance articles yet." : "No articles match your filters."}
        </div>
      )}
